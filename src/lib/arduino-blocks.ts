...
export const defineArduinoBlocks = () => {
  ...
  // --- BUCLE REPEAT UNTIL (Estilo VEX) ---
  //Blockly.Blocks['arduino_repeat_until'] = {
  //  init: function () {
  ...
  //    this.setColour(120); // Mismo color que los otros bucles
  //    this.setTooltip('Ejecuta las acciones repetidamente hasta que la condición sea verdadera.');
  //  }
  //};
  ...
  Blockly.Blocks['motor_setup'] = {
    init: function () {
      ...
      this.appendDummyInput()
        ...
        .appendField("EN")
        .appendField(new Blockly.FieldDropdown(DIGITAL_PINS_DROPDOWN), 'EN')
        .appendField("IN1")
        .appendField(new Blockly.FieldDropdown(DIGITAL_PINS_DROPDOWN), 'IN1')
        .appendField("IN2")
        .appendField(new Blockly.FieldDropdown(DIGITAL_PINS_DROPDOWN), 'IN2');

      this.setPreviousStatement(true, null);
      this.setNextStatement(true, null);
      this.setColour('#E67E22');
      this.setTooltip('Configuración del motor L298N. EN controla la velocidad.');
    }
  };
  ...
  Blockly.Blocks['motor_stop'] = {
    init: function () {
      ...
      this.appendDummyInput().appendField("Nombre").appendField(new Blockly.FieldDropdown(MOTOR_DROPDOWN), 'MOTOR')
      this.setPreviousStatement(true, null);
      this.setNextStatement(true, null);
      this.setColour('#E67E22');
    }
  ...
  Blockly.Blocks['ultrasonic_setup'] = {
    init: function () {
      this.appendDummyInput()
        .appendField("Configurar Ultrasonido")
      ...
      this.setTooltip('Configura los pines del sensor ultrasónico.');
    }
  };
  ...
  Blockly.Blocks['color_sensor_read'] = {
    init: function () {
      ...
      this.appendDummyInput().appendField("S0").appendField(new Blockly.FieldDropdown(DIGITAL_PINS_DROPDOWN), 'S0')
        .appendField("S1").appendField(new Blockly.FieldDropdown(DIGITAL_PINS_DROPDOWN), 'S1')
...
export const createArduinoGenerator = (): any => {
  ...
  Arduino.forBlock['motor_run'] = function (block: Blockly.Block) {
    ...
    const code = `digitalWrite(${name}_IN1, ${dir === 'FORWARD' ? 'HIGH' : 'LOW'});\n` +
      ...
      `analogWrite(${name}_EN, map(${speedPercentage}, 0, 100, 0, 255));\n`;
    return code;
  };
  ...
  // Sensores
  Arduino.forBlock['ultrasonic_setup'] = function (block: Blockly.Block) {
  ...
  // Repeat Until
  //Arduino.forBlock['arduino_repeat_until'] = function (block: Blockly.Block) {
  //  const condition = Arduino.valueToCode(block, 'CONDITION', Arduino.ORDER_NONE) || 'false';
  //  const branch = Arduino.statementToCode(block, 'DO');
  //  // Genera: while (!(condicion)) { ... }
...
export const arduinoToolbox = {
  ...
  contents: [
    ...
    },
    {
      ...
      colour: '#8E44AD',
      contents: [
        { kind: 'block', type: 'ultrasonic_setup' },
